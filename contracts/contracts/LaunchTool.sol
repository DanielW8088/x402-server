...
contract LaunchTool {
    ...
    event DebugBalances(uint256 token0Balance, uint256 token1Balance);
    event DebugApprovals(uint256 token0Approval, uint256 token1Approval);
    ...
    /**
     * @dev Safely approve tokens (compatible with USDC-like tokens that require zero approval first)
     */
    function _safeApprove(address token, address spender, uint256 amount) internal {
        IERC20 tokenContract = IERC20(token);
        uint256 currentAllowance = tokenContract.allowance(address(this), spender);
        
        // If current allowance is insufficient, reset to 0 first (USDC requirement)
    ...
    function configurePoolByAmount(
        ...
        emit DebugStep("Before transferFrom", 0);
        
        // Transfer tokens from user to this contract
        IERC20(token0).transferFrom(msg.sender, address(this), token0Amount);
        emit DebugStep("After token0 transferFrom", token0Amount);
        
        IERC20(token1).transferFrom(msg.sender, address(this), token1Amount);
        emit DebugStep("After token1 transferFrom", token1Amount);

        // Check balances
        uint256 balance0 = IERC20(token0).balanceOf(address(this));
        uint256 balance1 = IERC20(token1).balanceOf(address(this));
        emit DebugBalances(balance0, balance1);

        // Approve position manager to spend tokens (safely for USDC-like tokens)
        _safeApprove(token0, address(positionManager), token0Amount);
        emit DebugStep("After token0 approve", token0Amount);
        
        _safeApprove(token1, address(positionManager), token1Amount);
        emit DebugStep("After token1 approve", token1Amount);
        ...
        emit DebugApprovals(approval0, approval1);

        emit DebugStep("Before mint", 0);

        // Prepare mint parameters with 5% slippage tolerance (safer for initial LP)
        // Use unchecked to avoid overflow issues, but ensure result is never 0
        uint256 amount0Min;
        uint256 amount1Min;
        unchecked {
            amount0Min = (token0Amount * 95) / 100;
            amount1Min = (token1Amount * 95) / 100;
            // Ensure minimum is never 0 if desired amount is non-zero
            if (amount0Min == 0 && token0Amount > 0) amount0Min = 1;
            if (amount1Min == 0 && token1Amount > 0) amount1Min = 1;
        }
        ...
        emit DebugStep("After mint", positionId);
    ...
    function configurePoolByAmountWithSlippage(
        ...
        // Approve position manager (safely for USDC-like tokens)
        _safeApprove(token0, address(positionManager), token0Amount);
        _safeApprove(token1, address(positionManager), token1Amount);
